...
for(j in 1: length(TANvals)){
  ...
  suppByTAN[[j]] <- aggregate(Quantity ~ Due.Date + Orig.Qty, FUN = sum, data = suppByTAN[[j]])
}
  ...
  for(i in 1:length(bkByTAN[[j]]$Request.Date)){
    ...
    if (chron(bkByTAN[[j]]$Request.Date[i]) <= chron(bkByTAN[[j]]$LTgoal[i])){
      ...
      if ( chron(bkByTAN[[j]]$Request.Date[i]) <= chron(bkByTAN[[j]]$Cisco.Booked.Date[i]) + TP){
        ...
        closestDateIndex <- which.min(chron(bkByTAN[[j]]$Cisco.Booked.Date[i]) - chron(suppByTAN[[j]]$Due.Date))
        ...
        #closestDateIndex <- which.min(abs(chron(bkByTAN[[j]]$Cisco.Booked.Date[i]) - chron(suppByTAN[[j]]$Due.Date)))
        ...
        while(bkByTAN[[j]]$Ordered.Quantity[i] > 0){
          ...
          if (chron(bkByTAN[[j]]$Cisco.Booked.Date[i]) > chron(suppByTAN[[j]]$Due.Date[closestDateIndex])){
            ...
            if (suppByTAN[[j]]$Quantity[closestDateIndex] >= testbk$Ordered.Quantity[i]){
              ...
              suppByTAN[[j]]$Quantity[closestDateIndex] <- suppByTAN[[j]]$Quantity[closestDateIndex] - testbk$Ordered.Quantity[i]
              ...
              #then take out the quantity from the nearest backward bucket first; to do that, set that bucket supply to 0, and then subtract the available qty from the ordered qty
              testbk$Ordered.Quantity[i] <- testbk$Ordered.Quantity[i] - suppByTAN[[j]]$Quantity[closestDateIndex]
        ...
        #closestDateIndex <- which.min(abs(chron(bkByTAN[[j]]$Request.Date[i]) - chron(suppByTAN[[j]]$Due.Date)))
        closestDate <- suppByTAN[[j]]$Due.Date[closestDateIndex]
        #closestDateIndexInitial <- closestDateIndex
        ...
        if ((chron(bkByTAN[[j]]$Request.Date[i]) - TP) > chron(suppByTAN[[j]]$Due.Date[closestDateIndex])){
          ...
          print('CRD<=LT, CRD > CBD, CRD < LT, supply before CRD')
            ...
            #if there's not enough supply in the nearest supply bucket
            bkByTAN[[j]]$Ordered.Quantity[i] <- bkByTAN[[j]]$Ordered.Quantity - suppByTAN[[j]]$Quantity[closestDateIndex]
            suppByTAN[[j]]$Quantity[closestDateIndex] <- 0
            closestDateIndex <- closestDateIndex + 1
            print('CRD<=LT, CRD > CBD, CRD < LT, supply after CRD')
