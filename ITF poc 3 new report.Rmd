...
bkSDwCalc$LASnow <- ifelse( bkSDwCalc$CRDinLTflag == 'Y', ifelse( chron(bkSDwCalc$Promise.Date) > chron(bkSDwCalc$LTgoal), 0, 100), ifelse( chron(bkSDwCalc$Promise.Date) > chron(bkSDwCalc$Requested.Ship.Date), 0, 100) )

#calculate current OTS hit
bkSDwCalc$OTSnow <- ifelse( bkSDwCalc$CRDinLTflag == 'Y', ifelse( chron(bkSDwCalc$Ship.Date) > chron(bkSDwCalc$LTgoal), 0, 100), ifelse( chron(bkSDwCalc$Ship.Date) > chron(bkSDwCalc$Promise.Date), 0, 100) )
...
suppReOrg$Quantity <- as.numeric(suppReOrg$Quantity)
...
#TANvals <- c('37-1143-01', "69-2544-01" , "800-03191-01", "68-4697-02", "800-38968-01")
...
for(j in 1: length(TANvals)){
  ...
  bkByTAN[[j]] <- subset(bkSDwCalc, bkSDwCalc$TAN == TANvals[j])
...
for(j in 1: length(TANvals)){
  ...
  for(i in 1:length(bkByTAN[[j]]$Requested.Ship.Date)){
    ...
    print(paste('i = ', i))
    if (chron(bkByTAN[[j]]$Requested.Ship.Date[i]) <= bkByTAN[[j]]$LTgoal[i]){
      #if CRD <= LT
      if ( chron(bkByTAN[[j]]$Requested.Ship.Date[i]) <= chron(bkByTAN[[j]]$Cisco.Booked.Date[i]) + TP){
        ...
        if (chron(bkByTAN[[j]]$Cisco.Booked.Date[i]) > suppByTAN[[j]]$Due.Date[closestDateIndex]){
          ...
          if (suppByTAN[[j]]$Quantity[closestDateIndex] >= bkByTAN[[j]]$Ordered.Quantity[i]){
            ...
            endReached <- FALSE
            #then take out the quantity from the nearest backward bucket first; to do that, subtract the available qty from the ordered qty, then set that bucket supply to 
  ...
  write.xlsx(bkByTAN[[j]], file = "C:/Users/ali3/Documents/cisco/LAS project w Suman/ITF/analysis/newPD newLAS 12 TANs.xlsx", sheetName = paste("TAN", j, sep = ""), col.names = TRUE, row.names = FALSE, append = TRUE)
...
result <- data.table(1,2,3,4,5,6,7)
#then rename all the columns with following code
names(result) <- c("TAN", "as-is LAS", "to-be LAS", 'change in LAS', 'as-is OTS', 'to-be OTS', 'change in OTS')
...
for(j in 1:length(TANvals)){
  ...
  row2badd <- list(as.character(TANvals[j]), asisLAS, tobeLAS, changeLAS, asisOTS, tobeOTS, changeOTS)
...
##write the results table to the other TAN results sheets as well
  ...
  write.xlsx(result, file = "C:/Users/ali3/Documents/cisco/LAS project w Suman/ITF/analysis/newPD newLAS 12 TANs.xlsx", sheetName = "results table", col.names = TRUE, row.names = FALSE, append = TRUE)
